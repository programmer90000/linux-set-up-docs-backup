...
static void nm_secret_agent_new(GDBusMethodInvocation *inv, GVariant *params) {
    nm_secret_t *secret = g_malloc0(sizeof(nm_secret_t));

    secret->inv = g_object_ref(inv);
    g_variant_get(params, "(@a{sa{sv}}o&sasu)", NULL, &secret->path, NULL, NULL, NULL);
...
static void nm_conn_new_cb(GDBusConnection *con, GAsyncResult *res, gpointer d) {
    GVariant *result;
...
static void nm_conn_handle_cb(GDBusConnection *con, GAsyncResult *res, gchar *path) {
    nm_conn_t *conn;
...
static void nm_object_list_cb(GDBusConnection *con, GAsyncResult *res, gpointer data) {
    GVariant *result;
...
static void nm_name_disappeared_cb(GDBusConnection *con, const gchar *name, gpointer d) {
