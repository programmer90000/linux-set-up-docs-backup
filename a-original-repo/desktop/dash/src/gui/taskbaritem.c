...
static gboolean taskbar_item_action_exec (GtkWidget *self, gint slot, GdkEvent *ev) {
...
void taskbar_item_set_image (GtkWidget *icon, gchar *appid) {
    ...
    if ((ptr = strrchr(appid, '.')) && scale_image_set_image(icon, ptr+1, NULL)) {
        return;
    }
...
static void taskbar_item_decorate (GtkWidget *parent, GParamSpec *spec, GtkWidget *self) {
    TaskbarItemPrivate *priv;
...
static void taskbar_item_update (GtkWidget *self) {
    ...
    css_set_class(base_widget_get_child(self), "minimized", priv->win->state & WS_MINIMIZED);
    css_set_class(base_widget_get_child(self), "maximized", priv->win->state & WS_MAXIMIZED);
    css_set_class(base_widget_get_child(self), "fullscreen", priv->win->state & WS_FULLSCREEN);
    css_set_class(base_widget_get_child(self), "urgent", priv->win->state & WS_URGENT);
    css_set_class(base_widget_get_child(self), "focused", wintree_is_focused(priv->win->uid));

    if (priv->label && g_strcmp0(gtk_label_get_text(GTK_LABEL(priv->label)), priv->win->title)) {
    ...
    gtk_widget_unset_state_flags(base_widget_get_child(self), GTK_STATE_FLAG_PRELIGHT);
...
static void taskbar_item_class_init (TaskbarItemClass *kclass) {
    ...
    FLOW_ITEM_CLASS(kclass)->get_source = (void * (*)(GtkWidget *))taskbar_item_get_window;
...
GtkWidget *taskbar_item_new (window_t *win, GtkWidget *taskbar) {
    ...
    g_signal_connect(G_OBJECT(taskbar), "notify::labels", G_CALLBACK(taskbar_item_decorate), self);
    g_signal_connect(G_OBJECT(taskbar), "notify::icons", G_CALLBACK(taskbar_item_decorate), self);
    g_signal_connect(G_OBJECT(taskbar), "notify::title-width", G_CALLBACK(taskbar_item_decorate), self);
