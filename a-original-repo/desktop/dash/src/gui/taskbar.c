...
G_DEFINE_TYPE_WITH_CODE (Taskbar, taskbar, FLOW_GRID_TYPE, G_ADD_PRIVATE (Taskbar))

GtkWidget *taskbar_get_taskbar ( GtkWidget *shell, window_t *win, gboolean create ) {
    return shell;
}
...
static void taskbar_init ( Taskbar *self ) {
    g_object_set(G_OBJECT(self), "action", base_widget_attachment_new_array(parser_function_call_build("taskbaritemdefault"), 1, 0), NULL);
