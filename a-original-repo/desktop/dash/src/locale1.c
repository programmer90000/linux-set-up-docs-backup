...
static void locale1_cb ( GDBusConnection *con, GAsyncResult *res, void *data) {
...
static void locale1_changed_cb ( GDBusConnection *con, const gchar *sender, const gchar *path, const gchar *iface, const gchar *signal, GVariant *params, gpointer data ) {
    GVariant *dict;
    GVariantIter *iter;
    ...
    if(g_variant_lookup(dict, "Locale", "as", &iter)) {
        g_dbus_connection_call(con, locale_iface, locale_path, "org.freedesktop.DBus.Properties", "Get", g_variant_new("(ss)", locale_iface, "Locale"), NULL,
        G_DBUS_CALL_FLAGS_NONE, -1, NULL, (GAsyncReadyCallback)locale1_cb, NULL);
        g_variant_iter_free(iter);
    }
...
void locale1_init ( void ) {
    ...
    if( (con = g_bus_get_sync(G_BUS_TYPE_SYSTEM, NULL, NULL)) ) {
        g_dbus_connection_signal_subscribe(con, locale_iface,
            "org.freedesktop.DBus.Properties", "PropertiesChanged", NULL, NULL,  G_DBUS_SIGNAL_FLAGS_NONE, locale1_changed_cb, NULL, NULL);
    
    g_dbus_connection_call(con, locale_iface, locale_path, "org.freedesktop.DBus.Properties", "Get", g_variant_new("(ss)", locale_iface, "Locale"), NULL, G_DBUS_CALL_FLAGS_NONE, -1, NULL, (GAsyncReadyCallback)locale1_cb, NULL);
    }
}
