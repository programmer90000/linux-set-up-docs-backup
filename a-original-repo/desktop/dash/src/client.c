...
static void client_socket_connect_cb(GSocketClient *src, GAsyncResult *res, Client *client) {
    GSocket *sock;
...
gboolean client_socket_connect(Client *client) {
    ...
    if (strchr(client->src->fname, ':'))
        ...
        client->addr = (GSocketConnectable *)g_unix_socket_address_new(client->src->fname);
...
source_t *client_exec(gchar *fname, gchar *trigger) {
    ...
    client = g_malloc0(sizeof(Client));
    client->src = scanner_source_new(fname);
    client->connect = (gboolean (*)(struct _Client *))client_exec_connect;
    client->consume = client_source_update;
    client->trigger = trigger_name_intern(trigger);
    client_attach(client);
