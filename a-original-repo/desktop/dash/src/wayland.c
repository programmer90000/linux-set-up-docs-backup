...
gpointer wayland_iface_register ( const gchar *interface, guint32 min_ver, guint32 max_ver, const void *impl ) {
    GList *iter;
    wayland_iface_t *iface;
  ...
  for(iter=wayland_ifaces; iter; iter=g_list_next(iter)) {
      ...
      if(iface->version >= min_ver && !g_strcmp0(iface->iface, interface))
          return wl_registry_bind(wayland_registry, iface->global, impl, MIN(iface->version, max_ver));
      }
      return NULL;
}
