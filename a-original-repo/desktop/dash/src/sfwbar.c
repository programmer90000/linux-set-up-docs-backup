...
void parse_command_line(gint argc, gchar **argv) {
    ...
    static GOptionEntry entries[] = {
        ...
        {"socket", 's', 0, G_OPTION_ARG_FILENAME, &sockname, "Specify compositor IPC socket file"},
        ...
        {"monitor", 'm', 0, G_OPTION_ARG_STRING, &monitor, "Monitor to display the panel on (use \"-m list\" to list monitors`"},
        {"bar_id", 'b', 0, G_OPTION_ARG_STRING, &bar_id, "default sway bar_id to listen on for sway events"},
        {NULL}
    };
...
void log_print(const gchar *log_domain, GLogLevelFlags log_level, const gchar *message, gpointer data) {
    GDateTime *now;
    FILE *log_fd = stderr;
...
static void activate(GtkApplication* app, gpointer data) {
    ...
    g_thread_unref(g_thread_new("scanner", (GThreadFunc)base_widget_scanner_thread, g_main_context_get_thread_default()));

    vm_run_user_defined("SfwBarInit", NULL, NULL, NULL, NULL, base_widget_get_store(panel));
}
...
int main(int argc, gchar **argv) {
    ...
    g_unix_signal_add(SIGHUP, (GSourceFunc)sfwbar_restart, g_memdup2(argv, sizeof(gchar *) * (argc + 1)));
    signal_subscribe();
