...
static void sni_menu_about_to_show_cb(GDBusConnection *, GAsyncResult *, gpointer);
...
static void sni_menu_item_update(GtkWidget *item, GVariant *dict, GVariantIter *viter) {
    GtkWidget *submenu;
...
static GtkWidget *sni_menu_item_new(guint32 id, GVariant *dict, GtkWidget *prev) {
    GtkWidget *item;
    ...
    if (g_variant_lookup(dict, "type", "&s", &type) && !g_strcmp0(type, "separator")) {
...
static void sni_menu_get_layout_cb(GObject *src, GAsyncResult *res, gpointer data) {
    sni_item_t *sni = data;
    ...
    if (!(result = g_dbus_connection_call_finish(G_DBUS_CONNECTION(src), res, NULL))) {
        return;
    }
    ...
    if (!rev || (rev > GPOINTER_TO_INT(g_object_get_data(G_OBJECT(sni->menu_obj), "rev")))) {
...
static void sni_menu_about_to_show_cb(GDBusConnection *con, GAsyncResult *res, gpointer data) {
