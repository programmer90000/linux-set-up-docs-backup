...
static void scanner_var_parse_default(source_t *src, src_handler_t *handler, GString *str) {
    GList *iter;
    gchar *val;
...
static void scanner_handler_json_handle(source_t *src, src_handler_t *hnd, GString *str) {
    json_object *obj;
...
GIOStatus scanner_source_update(GIOChannel *in, source_t *src, gsize *size) {
    ...
    while ((status = g_io_channel_read_line_string(in, str, NULL, NULL)) == G_IO_STATUS_NORMAL) {
        if (size)
            *size += str->len;
        for (iter = src->handlers; iter; iter = g_list_next(iter)) {
    ...
    g_debug("scanner: input status %d, (%s)", status, src->fname ? src->fname : "(null)");
...
scan_var_t *scanner_var_new_calc(gchar *name, source_t *src, gpointer code, vm_store_t *store) {
    scan_var_t *var;
...
value_t scanner_get_value(GQuark id, gchar ftype, gboolean update, gboolean *vstate) {
    scan_var_t *var;
    value_t result;
