...
M.status = function(path, base_lookup, skip_bubbling, status_opts)
    ...
    local git_status =
        parser.parse_status_porcelain(status_porcelain_version, worktree_root, status_iter, skip_bubbling)
...
M.find_existing_status_code = function(path, base_lookup)
    ...
    if worktree_git_status then
        local worktree_status_code = find_status_code_in_git_status(worktree_git_status, worktree_root, path)
