...
local parsers = #_G.arg > 0 and { unpack(_G.arg) } or require("nvim-treesitter.config").get_installed("queries")
...
if #errors > 0 then
    ...
    for i, val in ipairs(timings) do
        print(string.format("%i. %.02fms %s %s", #timings - i + 1, val.duration * 1e-6, val.lang, val.query_type))
    end
    print("::endgroup::")
