...
local function do_generate(logger, repo, compile_location)
    ...
    logger:info(string.format("Generating parser.c from %s...", from_json and "grammar.json" or "grammar.js"))
...
M.update = a.async(function(languages, options)
    ...
    if #languages > 0 then
        return install(languages, { force = true, summary = summary, max_jobs = options and options.max_jobs })
    else
        if summary then
