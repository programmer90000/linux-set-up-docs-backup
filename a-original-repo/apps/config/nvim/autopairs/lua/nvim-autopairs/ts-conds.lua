...
conds.is_endwise_node = function(nodes)
    ...
    return function(opts)
        ...
        if target ~= nil and utils.is_in_table(nodes, target:type()) then
            ...
            if
                (begin_target ~= begin_parent and end_target == end_parent)
                or (end_parent - end_target == 1)
            then
                return true
            end
            -- return true
    ...
    return function(opts)
        ...
        for _, tree in ipairs(lang_tree:trees()) do
            ...
            if root and vim.treesitter.is_in_node_range(root, line, col) then
                ...
                local anonymous_node =
                    root:descendant_for_range(line, col, line, col)
...
conds.is_not_in_context = function()
    return function(opts)
        local context =
            require('nvim-autopairs.ts-utils').get_language_tree_at_position({
                utils.get_cursor(),
            })
        if context == nil then
            return
