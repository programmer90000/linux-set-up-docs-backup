...
_process_should_be_restored() {
    ...
    if is_pane_registered_as_existing "$session_name" "$window_number" "$pane_index"; then
        # Scenario where pane existed before restoration, so we're not restoring the proces either.
        return 1
...
# given full command: 'ruby /Users/john/bin/my_program arg1 arg2'and inline strategy: '~bin/my_program->my_program *' returns: 'arg1 arg2'
